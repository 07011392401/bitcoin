                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERACK) {
        ...
        if (m_txreconciliation) {
            if (!peer->m_wtxid_relay || !m_txreconciliation->IsPeerRegistered(pfrom.GetId())) {
                ...
                // - OR peer haven't sent us SENDRECON
                // Both can't happen later than here, and thus tx reconciliation won't be possible.
        ...
        for (CInv& inv : vInv) {
            ...
            if (inv.IsMsgBlk()) {
                ...
                if (m_txreconciliation && m_txreconciliation->IsPeerRegistered(pfrom.GetId()) && gtxid.IsWtxid()) {
                    m_txreconciliation->TryRemovingFromReconSet(pfrom.GetId(), gtxid.GetHash());
                }
            } else {
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (m_txreconciliation && m_txreconciliation->IsPeerRegistered(pfrom.GetId())) {
            m_txreconciliation->TryRemovingFromReconSet(pfrom.GetId(), wtxid);
        }
                ...
                if (peer->m_tx_relay->m_next_inv_send_time < current_time) {
                    ...
                    if (pto->IsInboundConn()) {
                        if (supports_recon) {
                            ...
                            peer->m_tx_relay->m_next_inv_send_time = GetExponentialRand(current_time,
                                INBOUND_INVENTORY_BROADCAST_INTERVAL_RECON);
                        } else {
                            peer->m_tx_relay->m_next_inv_send_time = NextInvToInbounds(current_time,
