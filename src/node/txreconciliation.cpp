...
class TxReconciliationTracker::Impl
{
        ...
        std::vector<uint32_t>& txs_to_request, std::vector<uint256>& txs_to_announce, std::optional<bool>& result)
    {
        ...
        if (recon_state.m_local_set.GetSize() > 0) {
        ...
        if (remote_sketch_capacity == 0 || !remote_sketch || !local_sketch) {
            ...
            txs_to_announce = recon_state.m_local_set.GetAllTransactions();
        ...
        if (local_sketch.Merge(remote_sketch).Decode(differences)) {
            ...
            recon_state.m_local_set.GetRelevantIDsFromShortIDs(differences, txs_to_request, txs_to_announce);
            ...
            recon_state.m_state_init_by_us.m_phase = Phase::EXT_REQUESTED;
    ...
    void TryRemovingFromReconSet(NodeId peer_id, const uint256 wtxid_to_remove)
    {
        ...
        if (recon_state.m_local_set_snapshot.m_wtxids.find(wtxid_to_remove) !=
            recon_state.m_local_set_snapshot.m_wtxids.end()) {
    ...
    void RespondToInitialRequest(ReconciliationState& recon_state, const NodeId peer_id, std::vector<uint8_t>& skdata)
    {
    ...
    void RespondToExtensionRequest(ReconciliationState& recon_state, NodeId peer_id, std::vector<uint8_t>& skdata)
    {
        assert(recon_state.m_capacity_snapshot > 0);
        // Update local reconciliation state for the peer.
        recon_state.m_state_init_by_them.m_phase = Phase::EXT_RESPONDED;
        ...
        std::vector<uint32_t>& txs_to_request, std::vector<uint256>& txs_to_announce, std::optional<bool>& result)
    {
        ...
        Phase cur_phase = recon_state.m_state_init_by_us.m_phase;
    ...
    void HandleExtensionRequest(NodeId peer_id)
    {
        ...
        LOCK(m_mutex);
        auto& recon_state = std::get<ReconciliationState>(m_states.find(peer_id)->second);
        if (recon_state.m_state_init_by_them.m_phase != Phase::INIT_RESPONDED) return;
        ...
        recon_state.m_state_init_by_them.m_phase = Phase::EXT_REQUESTED;
        ...
        const std::vector<uint32_t>& remote_missing_short_ids, std::vector<uint256>& remote_missing)
    {
        ...
        if (recon_state.m_state_init_by_them.m_phase != Phase::INIT_RESPONDED &&
            recon_state.m_state_init_by_them.m_phase != Phase::EXT_RESPONDED) return false;
        ...
        if (recon_result) {
            remote_missing = recon_state.m_local_set_snapshot.GetWTXIDsFromShortIDs(remote_missing_short_ids);
        } else {
            ...
            remote_missing = recon_state.m_local_set_snapshot.GetAllTransactions();
        }
        ...
        recon_state.m_state_init_by_them.m_phase = Phase::NONE;
